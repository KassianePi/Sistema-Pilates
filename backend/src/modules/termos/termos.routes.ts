...
import {
  listar, buscarPorId, criar, editar, publicar, listarAceites,
  obterStatus, aceitar, meusAceites,
...
export async function termosRoutes(fastify: FastifyInstance) {
  ...
  fastify.get('/api/v1/aluno/termos/meus-aceites', { onRequest: [authenticateToken, requireRole('ALUNO')] }, meusAceites)
  ...
  fastify.post('/api/v1/termos/:id/publicar', { onRequest: [authenticateToken, authorize('termos', 'update')] }, publicar)
  fastify.get('/api/v1/termos/:id/aceites', { onRequest: [authenticateToken, authorize('termos', 'read')] }, listarAceites)
}
