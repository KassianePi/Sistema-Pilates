...
describe('AgendaService', () => {
  ...
  describe('reagendar', () => {
    it('deve reagendar guardando a data anterior', async () => {
      ...
      await service.reagendar('aula-1', 'user-1', {
        dataHoraInicio: nova,
        justificativa: JUST.justificativa,
      })
      expect(mockRepo.update).toHaveBeenCalledWith(
        'aula-1',
        expect.objectContaining({
          dataHoraAnterior: antes,
          dataHoraInicio: new Date(nova),
        }),
      )
    })
    ...
    it('deve lançar conflito ao reagendar para horário ocupado', async () => {
      ...
      const nova = new Date(Date.now() + 172800000).toISOString()
      await expect(
        service.reagendar('aula-1', 'user-1', {
          dataHoraInicio: nova,
          justificativa: JUST.justificativa,
        }),
      ).rejects.toThrow()
