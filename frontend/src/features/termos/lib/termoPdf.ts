...
export async function gerarTermoPdf(
  ...
  if (logo) {
    try {
      doc.addImage(logo, 'PNG', margin, y, 52, 52)
    } catch {
      /* logo opcional */
    }
  }
  ...
  doc.setFontSize(15)
  doc.setTextColor(64, 62, 60)
  doc.text(termo.titulo, margin, y)
  y += 18
  doc.setFont('helvetica', 'normal')
  doc.setFontSize(9)
  ...
  doc.text(
    `Versão ${termo.versao}  ·  Publicado em ${pub}  ·  Gerado em ${new Date().toLocaleDateString('pt-BR')}`,
    margin,
    y,
  )
  y += 22
  ...
  doc.text(POLITICA_REEMBOLSO.titulo, margin + padding, ty)
  ty += tituloH
  doc.setFont('helvetica', 'normal')
  doc.setFontSize(10)
  doc.setTextColor(120, 90, 30)
  for (const l of linhasReembolso) {
    doc.text(l, margin + padding, ty)
    ty += lineH
  }
  y += boxH + 18
