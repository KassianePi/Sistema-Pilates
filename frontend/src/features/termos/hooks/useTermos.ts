...
export function useAceitarTermo() {
  ...
  return useMutation({
    ...
    onError: (err) =>
      toast.error(msgErro(err, "Não foi possível registrar o aceite.")),
  });
}
...
export function useCriarTermo() {
  const qc = useQueryClient();
  return useMutation({
    mutationFn: (payload: { titulo: string; conteudo: string }) =>
      termosService.criar(payload),
    onSuccess: () => {
...
export function usePublicarTermo() {
  ...
  return useMutation({
    ...
    onSuccess: () => {
      toast.success(
        "Versão publicada! Os alunos verão esta versão para aceite.",
      );
      qc.invalidateQueries({ queryKey: ADMIN_KEY });
      qc.invalidateQueries({ queryKey: STATUS_KEY });
    },
