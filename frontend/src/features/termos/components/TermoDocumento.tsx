...
function Corpo({ conteudo }: { conteudo: string }) {
  ...
  const fecharLista = (key: string) => {
    ...
    elementos.push(
      <ul
        key={key}
        className="list-disc pl-5 space-y-1 text-sm text-cinza-texto leading-relaxed"
      >
        {lista.map((item, i) => (
          <li key={i}>
            <Inline texto={item} />
  ...
  blocos.forEach((b, i) => {
    ...
    if (b.tipo === "h2") {
      elementos.push(
        <h2
          key={i}
          className="text-base font-bold text-roxo-profundo mt-5 mb-1.5"
        >
          {b.texto}
        </h2>,
      );
      ...
      elementos.push(
        <h3
          key={i}
          className="text-sm font-semibold text-cinza-forte mt-4 mb-1"
        >
          {b.texto}
        </h3>,
      );
...
export function TermoDocumento({ termo }: { termo: TermoUso }) {
  const publicado = termo.publicadoEm
    ? new Date(termo.publicadoEm).toLocaleDateString("pt-BR")
    : null;
    ...
    <article className="bg-branco-puro">
      ...
      <header className="flex items-center gap-4 pb-4 border-b-2 border-ouro-clinica">
        ...
        <div className="min-w-0">
          <h1 className="text-lg font-bold text-cinza-forte leading-tight">
            {CLINICA.nome}
          </h1>
          <p className="text-xs text-cinza-texto uppercase tracking-wider">
            {CLINICA.subtitulo}
          </p>
          <p className="text-xs text-cinza-texto mt-0.5">
            Proprietária: {CLINICA.proprietaria}
          </p>
        </div>
      </header>
