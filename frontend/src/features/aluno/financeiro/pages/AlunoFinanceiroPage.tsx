...
export function AlunoFinanceiroPage() {
  ...
  const abaInicial: Aba =
    abaParam === 'comprovantes' || abaParam === 'reembolsos' || abaParam === 'avulsa' ? abaParam : 'resumo'
  ...
  const comComprovante = useMemo(
    () =>
      new Set(
        comprovantes.filter((c) => c.status === 'PENDENTE' || c.status === 'APROVADO').map((c) => c.mensalidadeId),
      ),
    [comprovantes],
  )
  ...
  const elegiveisReembolso = mensalidades.filter(
    (m) => (m.status === 'PAGO' || m.status === 'PARCIAL') && !comEstorno.has(m.id),
  )
  ...
  const mensalidadeAtual =
    pendentes.slice().sort((a, b) => new Date(a.vencimento).getTime() - new Date(b.vencimento).getTime())[0] ??
    mensalidades[0] ??
    null
        ...
        <div className="space-y-6">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
            ...
            <KpiCard
              ...
              value={proximaCobranca ? formatarData(proximaCobranca.vencimento) : '—'}
              hint={proximaCobranca ? formatarValor(proximaCobranca.valor) : 'Nada em aberto'}
            />
            <KpiCard
                ...
                {mensalidades.map((m) => (
                  <li key={m.id} className="flex items-center justify-between gap-3 px-6 py-3 flex-wrap">
                    <div className="min-w-0">
                      ...
                      <p className="text-xs text-cinza-texto">Vencimento: {formatarData(m.vencimento)}</p>
                      {m.status === 'PAGO' && m.pagamentos && m.pagamentos.length > 0 && (
            ...
            {pendentes.length === 0 ? (
              ...
              <>
                <p className="text-xs text-cinza-texto px-6 pt-4">
                  Pague via PIX e <strong>envie o comprovante</strong> para o studio confirmar. Sem o arquivo em mãos?
                  Use <strong>Avisar que paguei</strong> só para sinalizar o pagamento.
                </p>
                      ...
                      <li key={m.id} className="flex items-center justify-between gap-3 px-6 py-3 flex-wrap">
                        <div className="min-w-0">
                          ...
                          <p className="text-xs text-cinza-texto">
                            {formatarValor(m.valor)} · vence {formatarData(m.vencimento)}
                          </p>
                        </div>
                        <div className="flex items-center gap-2 flex-wrap justify-end">
                          <Button
                            ...
                            onClick={() => setNotificarModal({ id: m.id, nomePlano })}
                          >
                ...
                {comprovantes.map((c) => (
                  <li key={c.id} className="flex items-start justify-between gap-3 px-6 py-3 flex-wrap">
                    <div className="min-w-0">
                      ...
                      <p className="text-xs text-cinza-texto">{c.nomeArquivo}</p>
        ...
        <SectionCard title="Aula Avulsa" icon={Zap}>
          <div className="flex flex-col items-start gap-4">
            <p className="text-sm text-cinza-texto">
              Sem plano mensal ou quer uma aula extra? Solicite uma aula avulsa ao studio. O administrador criará a
              cobrança e confirmará a data com você.
            </p>
            <Button
