    ...
    <div className="space-y-6">
      ...
      {isLoading ? (
        ...
        <SectionCard>
          <EmptyState
            icon={FileText}
            message="Nenhum termo de uso publicado no momento."
          />
        </SectionCard>
      ) : (
        <>
          ...
          >
            {status?.aceito ? (
              <p className="flex items-center gap-2 text-sm text-cinza-forte">
                ...
                Você aceitou a versão {status.versaoAceita} em{" "}
                {formatarDataHora(status.aceitoEm)}.
              </p>
            ) : (
              <p className="text-sm text-amber-700">
                Aceite pendente para a versão {status?.versaoAtual}.
              </p>
            )}
          </SectionCard>
          ...
          <SectionCard title="Histórico de versões aceitas" icon={History}>
            {aceites.length === 0 ? (
              <EmptyState
                icon={History}
                message="Nenhum aceite registrado ainda."
              />
            ) : (
