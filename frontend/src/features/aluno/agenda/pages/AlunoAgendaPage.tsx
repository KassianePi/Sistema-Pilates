...
import { CalendarDays, Clock, User, Sparkles, History, X, AlertCircle, CalendarClock } from 'lucide-react'
...
function filtrar(aulas: Aula[], status: string, ini: string, fim: string): Aula[] {
...
function AulaCard({ aula, onClick }: { aula: Aula; onClick: () => void }) {
  ...
  const alterada = ['CANCELADA', 'SUSPENSA', 'EXCLUIDA', 'ADIADA'].includes(aula.status)
  return (
        ...
        <CardContent className="p-5">
          <div className="flex items-start justify-between gap-4">
            <div className="space-y-1 min-w-0">
              ...
              <p className="text-sm text-cinza-texto capitalize">{formatarDataLonga(aula.data)}</p>
            ...
            <div className="flex flex-col items-end gap-2 shrink-0">
              ...
              <span className={cn('text-[11px] px-2 py-0.5 rounded-full border font-medium', tag.className)}>
                {tag.label}
              </span>
            </div>
          </div>
          ...
          {aula.dataHoraAnterior && (
            <p className="mt-3 text-xs text-amber-700 flex items-center gap-1.5">
              <CalendarClock className="w-3.5 h-3.5" /> Reagendada — toque para ver as datas
            </p>
          )}
          {alterada && aula.justificativa && (
            <p className="mt-2 text-xs text-cinza-texto flex items-start gap-1.5">
              <AlertCircle className="w-3.5 h-3.5 mt-0.5 text-rosa-vibrante shrink-0" /> {aula.justificativa}
            </p>
          )}
          {aula.observacoes && <p className="mt-2 text-xs text-cinza-texto line-clamp-2">Obs.: {aula.observacoes}</p>}
        </CardContent>
      </Card>
    </button>
  )
...
export function AlunoAgendaPage() {
  ...
  return (
    <div className="space-y-6">
      <PageHeader title="Minhas Aulas" subtitle="Agenda, grade aberta e histórico." icon={CalendarDays} />
      ...
      {isLoading ? (
        ...
        <div className="space-y-3">
          {aulas.map((aula) => (
            <AulaCard key={aula.id} aula={aula} onClick={() => setDetalhe(aula)} />
          ))}
        </div>
      )}
