...
import { formatarData, formatarDataHora, formatarValor, primeiroNome } from '../../utils/format'
      ...
      >
        <div className="min-w-0">
          ...
          <p className="text-xs text-cinza-texto">
            {formatarData(aula.data)} · {aula.horaInicio} · {aula.professor.usuario.nomeCompleto}
          </p>
        </div>
      ...
      {isLoading ? (
        ...
        <>
            ...
            >
              <span
                className={cn(
                  ...
                  temVencido ? 'bg-rosa-vibrante/15 text-rosa-vibrante' : 'bg-amber-100 text-amber-600',
                )}
              >
              ...
              <div className="flex-1 min-w-0">
                <p className="font-semibold text-cinza-forte">
                  {temVencido ? 'Você tem mensalidade vencida' : 'Mensalidade em aberto'}
                </p>
                <p className="text-sm text-cinza-texto">
                  {qtdEmAberto > 1 ? `${qtdEmAberto} cobranças` : '1 cobrança'} · {formatarValor(totalEmAberto)} — envie
                  o comprovante para regularizar.
                </p>
              </div>
          ...
          <section className="space-y-4">
            <h2 className="text-sm font-semibold text-cinza-texto uppercase tracking-wider">Resumo</h2>
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
              <KpiCard label="Aulas realizadas no mês" icon={CheckCircle2} tone="success" value={aulasRealizadasMes} />
              <KpiCard
          ...
          {/* Ações rápidas */}
          <section>
            <h2 className="text-sm font-semibold text-cinza-texto uppercase tracking-wider mb-4">Ações rápidas</h2>
            <QuickActions actions={quickActions} />
          </section>

          {/* Minha agenda — Próxima aula / Minhas aulas / Grade geral */}
          <section className="space-y-4">
            <h2 className="text-sm font-semibold text-cinza-texto uppercase tracking-wider">Minha agenda</h2>
                ...
                >
                  <div className="min-w-0">
                    ...
                    <p className="text-sm text-cinza-texto capitalize">
                      {formatarData(proximaAula.data)} · {proximaAula.horaInicio}
                    </p>
                    <p className="text-xs text-cinza-texto mt-0.5">{proximaAula.professor.usuario.nomeCompleto}</p>
                  </div>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
              <SectionCard
                ...
                icon={ClipboardCheck}
                action={
                  <Link to="/aluno/agenda" className="text-xs text-lilas-medio hover:underline flex items-center gap-1">
                    ...
                    <li key={n.id}>
                      ...
                      >
                        ...
                        <div className="min-w-0 flex-1">
                          ...
                          <p className="text-xs text-cinza-texto mt-0.5">{formatarDataHora(n.createdAt)}</p>
                        </div>
                      </button>
                    </li>
                  )
