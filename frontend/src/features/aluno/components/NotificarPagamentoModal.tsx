...
export function NotificarPagamentoModal({
  mensalidadeId,
  nomePlano,
  onClose,
}: NotificarPagamentoModalProps) {
  const [observacoes, setObservacoes] = useState("");
  const notificar = useNotificarPagamento();

  function submit() {
    notificar.mutate(
      { mensalidadeId, observacoes: observacoes.trim() || undefined },
      { onSuccess: onClose },
    );
  }
        ...
        <div className="space-y-4 mt-2">
          <div className="bg-lilas-claro/40 border border-lilas-medio/20 rounded-lg p-3 space-y-1 text-sm">
            ...
            <p className="text-cinza-texto">
              Após pagar via PIX, avise o studio. A confirmação é feita
              manualmente.
            </p>
          </div>
          <div className="space-y-1.5">
            <Label>
              Observação{" "}
              <span className="text-cinza-texto text-xs">(opcional)</span>
            </Label>
            <Textarea
