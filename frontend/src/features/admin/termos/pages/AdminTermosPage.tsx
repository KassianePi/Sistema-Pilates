...
function FormVersaoModal({
  ...
  const [titulo, setTitulo] = useState(
    termo?.titulo ?? "Termos de Uso e Prestação de Serviços",
  );
    ...
    <Dialog open onOpenChange={(v) => !v && onClose()}>
      <DialogContent className="max-w-2xl max-h-[88vh] flex flex-col">
        <DialogHeader>
          <DialogTitle>
            {isEditing
              ? `Editar versão ${termo!.versao} (rascunho)`
        ...
        <div className="space-y-4 mt-2 overflow-y-auto flex-1 pr-1">
          ...
          <div className="space-y-1.5">
            ...
            <p className="text-xs text-cinza-texto">
              ...
              <code>- item</code> de lista e <code>**negrito**</code>. A
              política de reembolso (7 dias úteis) e os dados da clínica
              aparecem automaticamente em destaque.
            </p>
          </div>
        </div>
      ...
      <DialogContent className="max-w-lg max-h-[80vh] flex flex-col">
        <DialogHeader>
          <DialogTitle className="flex items-center gap-2">
            <Users className="w-4 h-4 text-roxo-profundo" /> Aceites da versão{" "}
            {termo.versao}
          </DialogTitle>
        </DialogHeader>
        <div className="overflow-y-auto flex-1 -mt-1">
          {isLoading ? (
            <p className="text-sm text-cinza-texto text-center py-8">
              Carregando...
            </p>
          ) : aceites.length === 0 ? (
            <p className="text-sm text-cinza-texto text-center py-8">
              Nenhum aluno aceitou esta versão ainda.
            </p>
          ) : (
                ...
                >
                  <div className="min-w-0">
                    <p className="text-sm font-medium text-cinza-forte truncate">
                      {a.aluno.usuario.nomeCompleto}
                    </p>
                    <p className="text-xs text-cinza-texto truncate">
                      {a.aluno.usuario.email}
                    </p>
                  </div>
                  <span className="text-xs text-cinza-texto shrink-0">
                    {fmtDataHora(a.aceitoEm)}
                  </span>
                </li>
              ))}
            </ul>
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between gap-3 flex-wrap">
        <div>
          ...
          <p className="text-sm text-cinza-texto mt-1">
            Crie, edite e publique as versões do termo. Versões publicadas são
            imutáveis.
          </p>
        </div>
      ...
      <Card>
        ...
        <CardContent>
          {isLoading ? (
            <p className="text-cinza-texto text-sm py-6 text-center">
              Carregando...
            </p>
          ) : termos.length === 0 ? (
            <div className="flex flex-col items-center py-10 text-cinza-texto">
              ...
              <p className="text-sm">
                Nenhuma versão cadastrada. Crie a primeira versão.
              </p>
            </div>
          ) : (
            <ul className="divide-y divide-bege-cartao">
                ...
                >
                  <div className="min-w-0 flex-1">
                    ...
                    <p className="text-xs text-cinza-texto mt-0.5">
                      {t.publicado
                        ? `Publicada em ${fmtData(t.publicadoEm)}`
                        : `Criada em ${fmtData(t.criadoEm)}`}
                    </p>
                  </div>
      ...
      {editando && (
        <FormVersaoModal termo={editando} onClose={() => setEditando(null)} />
      )}
      {previewing && (
        <PreviewModal termo={previewing} onClose={() => setPreviewing(null)} />
      )}
      {vendoAceites && (
        <AceitesModal
          termo={vendoAceites}
          onClose={() => setVendoAceites(null)}
        />
      )}
      ...
      >
        <AlertDialogContent>
          <AlertDialogHeader>
            <AlertDialogTitle>
              Publicar versão {publicando?.versao}?
            </AlertDialogTitle>
            <AlertDialogDescription>
              Esta versão passará a ser a vigente e será apresentada aos alunos
              para aceite. Após publicada, ela não poderá mais ser editada. As
              demais versões serão despublicadas.
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
