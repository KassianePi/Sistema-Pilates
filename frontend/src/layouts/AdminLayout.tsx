  ...
  return (
    <>
        ...
        >
          <div className="flex flex-col items-center gap-5 animate-success-expand">
            ...
            <div className="text-center">
              <p className="text-preto-silhueta font-semibold text-lg tracking-wide">Até logo!</p>
              <p className="text-cinza-silhueta text-sm mt-1">Saindo com segurança...</p>
            </div>
          </div>
        </div>
      )}
      ...
      <div className="flex h-screen bg-creme-fundo overflow-hidden">
        {/* Overlay mobile */}
        {mobileOpen && (
          <div className="fixed inset-0 bg-preto-puro/40 z-20 md:hidden" onClick={() => setMobileOpen(false)} />
        )}
          ...
          <div className="flex items-center justify-between h-16 px-4 border-b border-white/10 flex-shrink-0">
            {!collapsed && (
              <div className="overflow-hidden">
                <p className="font-bold text-branco-puro text-sm leading-tight truncate">Studio de Pilates</p>
          ...
          <div className="flex-shrink-0 border-t border-white/10 p-3 space-y-1">
            {!collapsed && adminUser && (
              <div className="px-3 py-2 mb-2">
                <p className="text-xs font-medium text-branco-puro truncate">{adminUser.nome}</p>
        ...
        <div className="flex-1 flex flex-col min-w-0 overflow-hidden">
          ...
          <header className="md:hidden flex items-center h-14 px-4 bg-branco-puro border-b border-bege-cartao flex-shrink-0">
            <button onClick={() => setMobileOpen(true)} className="p-2 rounded-md text-cinza-texto hover:bg-bege-suave">
              <Menu className="w-5 h-5" />
            </button>
            <span className="ml-3 font-semibold text-cinza-forte text-sm">Studio de Pilates</span>
          </header>
